# Scaffolding EF Core models from Oracle views based on .env and optional mapping file
# Usage:
#   .\Generate-Scaffold-Projet.ps1 -tables "O_VIEW_INSTITUTION_SECTORIELLE"
#   .\Generate-Scaffold-Projet.ps1 -mappingFile mapping.json
...
$infraProj = Join-Path $PSScriptRoot '..\Programmation.Infrastructure\Programmation.Infrastructure.csproj'
$apiProj   = Join-Path $PSScriptRoot '..\Programmation.API\Programmation.API.csproj'

if (!(Test-Path $infraProj)) { Write-Error "Programmation Infrastructure introuvable : $infraProj"; exit 1 }
if (!(Test-Path $apiProj))   { Write-Error "Programmation API introuvable : $apiProj"; exit 1 }
...
foreach ($tbl in $tableList) {
    ...
    $efParams = @(
        ...
        '--context-dir', 'Data',
        '--output-dir', "..\Programmation.Infrastructure\Entities",
        '--context', 'ProgrammationDbContext',
        '--table', $tbl,
        '--data-annotations',
        '--force'
    )
