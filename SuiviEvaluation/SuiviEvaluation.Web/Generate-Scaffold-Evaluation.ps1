...
$infraProj = Join-Path $PSScriptRoot '..\SuiviEvaluation.Infrastructure\SuiviEvaluation.Infrastructure.csproj'
$apiProj   = Join-Path $PSScriptRoot '..\SuiviEvaluation.API\SuiviEvaluation.API.csproj'

if (!(Test-Path $infraProj)) { Write-Error "SuiviEvaluation Infrastructure introuvable : $infraProj"; exit 1 }
if (!(Test-Path $apiProj))   { Write-Error "SuiviEvaluation API introuvable : $apiProj"; exit 1 }
...
foreach ($tbl in $tableList) {
    ...
    $efParams = @(
        ...
        '--context-dir', 'Data',
        '--output-dir', "..\SuiviEvaluation.Infrastructure\Entities",
